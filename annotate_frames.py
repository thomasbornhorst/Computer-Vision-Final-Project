...
def draw_bb(frame_img, bb, pred_class):
    ...
    #return cv2.rectangle(frame_img, (int(bb.iloc[1]), int(bb.iloc[2])), (int(bb.iloc[3]), int(bb.iloc[0])), color, 2) # for making annotating true boxes
...
def annotate_frames(predictions_df, pred_classes_df, test_df, frame_img_path, new_img_path):
    for frame_ind in range(len(predictions_df)):
        ...
        print(frame_path)
        frame_predictions = predictions_df.iloc[frame_ind,]
        #frame_predictions = test_df.iloc[frame_ind,] # for making annotating true boxes
        pred_classes = pred_classes_df.iloc[frame_ind,]

        frame_img = cv2.imread(frame_path)

        for annotation_ind in range(len(pred_classes)-1):
        #for annotation_ind in range(23): # for making annotating true boxes
            pred_bb = frame_predictions.iloc[annotation_ind*4+1:annotation_ind*4+5]
            pred_class = pred_classes.iloc[annotation_ind]
            if annotation_ind < 11: pred_class = 1
            elif annotation_ind < 22: pred_class = 2
            else: pred_class = 3
...
def annotate_blank_frames(predictions_df, test_df, new_img_path):
    for frame_ind in range(len(predictions_df)):
        ...
        frame_annotations = test_df.iloc[frame,]

        frame_img = np.full((2160,3840,3), 255, dtype=np.uint8)

        for annotation_ind in range(len(frame_predictions)//4-1):
            pred_bb = frame_predictions.iloc[annotation_ind*4+1:annotation_ind*4+5]
            frame_img = draw_blank_bb(frame_img, pred_bb)

        for annotation_ind in range(23):
...
def main():
    ...
    blank_results__dir = './predictions/blank_viz_results'
    ...
    annotate_blank_frames(predictions_df, test_df, blank_results__dir)
